...
public class LEG_Brain {
	...
	LEG_Brain(int inputLayerSize, int layerCount, int nodeCount, int outputLayerSize) {
		// This will hold a temporary array of unique nodes generated with the appropriate size
		LEG_Node[] tmpNodeArray;
		
		// Making the input layer:
		LEG_Layer tmpInputLayer;
		tmpNodeArray = new LEG_Node[inputLayerSize];
		// For as many nodes as there should be in the input layer, make a new node with zero inputs...
	...
	public void train(double[][] trainingData) {
		...
		for (int i = 0; i < students.length; i++) {
			...
			for (int j = 0; j < trainingData.length; j++) {
				...
				// If answer is equal to the correct answer stored in the training data add one to correctTests
		...
		// For every single student
		for (int i = 0; i < students.length; i++) {	
			// If maxValue is less than or equal to the given student's fitness, make it the new max
			// This is done so that if most have the same score, the one with the most extreme changes will be chosen
	...
	private void mutate(LEG_Network subject, int mutationFactor) {
		// how many layers the subject has - the amount of hidden layers plus one for the output layer
		...
		for (int i = 0; i < (int) (mutationFactor / 100 + 1); i++) {
			// Deciding what layer to modify (not including the input layer because that has no weights/bias)

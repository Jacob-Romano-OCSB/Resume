 ...
 * Course: ICS3U
 * AISandbox.java
 * Purpose: To create a tool to experiment with creating and growing artificial intelligences
 */
...
public class LEG_AISandbox extends Application {
	...
	public void start(Stage stage) {
        ...
        root.setBackground(new Background(new BackgroundFill(Paint.valueOf("888888"), CornerRadii.EMPTY, Insets.EMPTY)));
	...
	public void updateGraph() {
		...
		for (int i = 0; i < GRAPH_WIDTH; i++) {
		    for (int j = 0; j < GRAPH_HEIGHT; j++) {
		        ...
		        decision = brain.evaluate(new double[] { 
		        		((double) i / GRAPH_WIDTH) * 2 - 1, // Formats from pixel space to between -1 and 1
	...
	private void compileBrain() {
		...
		try {
			...
			// Sets the layer count to be the value of txtNodeCount if the layerCount is not zero
			int nodeCount = Integer.parseInt(txtNodeCount.getText());;
			// If layerCount is not zero
			if (layerCount != 0) {
				// if nodeCount is zero or less, set nodeCount to be one and update the text field
				if (nodeCount <= 0) {
					nodeCount = 1;
			...
			// Output layer size is equal to how many marker types have been programmed in, 2 for now
			...
			// Sets the compile error to be invisible in case the last compile attempt ended in an error
			lblCompileError.setVisible(false);
		} catch (Exception e) {
